...
const ProfessionalCard = ({ professional, onLearnMore }: ProfessionalCardProps) => (
  <div className="bg-card rounded-xl overflow-hidden card-shadow hover:card-shadow-hover transition-all duration-300 group text-center">
    ...
    <div className="p-6">
      ...
      {professional.role && (
        <p className="text-sm text-muted-foreground mb-4">{professional.role}</p>
      )}
      <Button onClick={() => onLearnMore(professional)} variant="outline" className="w-full">
        SAIBA MAIS
      </Button>
    </div>
  </div>
        ...
        <DialogDescription asChild>
          <div className="space-y-6 mt-4">
            {professional.curriculum.length > 0 &&
              !professional.curriculum.includes("Pendente") && (
                <div>
...
const ProfessionalsSection = () => {
  ...
  const visibleProfessionals = professionals.filter((item) => item.showOnHome !== false);
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center mb-12">
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">
            Conheca os profissionais que fazem parte da OncoAssist
          </p>
        </div>
